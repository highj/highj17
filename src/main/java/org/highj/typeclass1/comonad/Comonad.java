...
public interface Comonad<W> extends Extend<W> {
    ...
    //(=>>)
    default <A, B> __<W, B> unbind(__<W, A> nestedA, Function<__<W, A>, B> fn) {
        return extend(fn).apply(nestedA);
    }

    //(.>>)
    default <A, B> __<W, B> inject(__<W, A> nestedA, B b) {
    ...
    default <A> List<__<W, A>> parallelW(__<W, List<A>> nestedList) {
        return mapW(Function.<__<W, A>>identity()).apply(nestedList);
    }

    default <A,B> Function<__<W,A>, Stream<B>> unfoldW(final Function<__<W, A>, T2<B,A>> fn) {
        return nestedA -> {
   ...
   default <A, B> List<B> sequenceW(List<Function<__<W, A>, B>> fnList, __<W, A> nestedA) {
        List<B> listB = List.Nil();
