...
public class Set<A> implements __<Set.µ, A>, Iterable<A>, Predicate<A> {
    ...
    public boolean test(A value) {
        ...
        return switch (Ordering.compare(vhc, hc)) {
            case LT -> left.test(value);
            case GT -> right.test(value);
            case EQ -> bucket.contains(value);
        };
    }
    ...
    public Set<A> plus(A a) {
        ...
        return switch (Ordering.compare(ahc, hc)) {
            case EQ -> bucket.contains(a) ? this : withBucket(bucket.plus(a));
            case LT -> withLeft(left.plus(a));
            case GT -> withRight(right.plus(a));
        };
    }
        ...
        return switch (Ordering.compare(ahc, hc)) {
            case EQ -> {
                ...
                if (!newBucket.isEmpty()) {
                    ...
                    yield  new Set<>(pair._1().hc, pair._1().bucket, left, pair._2());
                }
            }
            case LT -> withLeft(left.minus(a));
            case GT -> withRight(right.minus(a));
        };
    }
    ...
    public static <A> Monoid<Set<A>> monoid() {
        return Monoid.create(Set.empty(), Set::plus);
    }
}
