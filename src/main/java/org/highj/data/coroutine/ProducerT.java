 ...
 */
public record ProducerT<E,M,A>(FreeT<__<YieldF.µ,E>,M,A> toFreeT) implements __3<ProducerT.µ,E,M,A> {
    public interface µ {
    }
    ...
    public static <E> Iterator<E> toIterator(ProducerT<E,T1.µ,T0> generator) {
        Maybe<T2<E,ProducerT<E,T1.µ,T0>>> initState = asT1(generator.run(T1.monadRec))._1().maybeRight();
        return new Iterator<>() {
