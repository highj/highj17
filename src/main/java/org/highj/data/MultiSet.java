...
public class MultiSet<A> implements __<MultiSet.µ, A>, Iterable<T2<A, Integer>>, Function<A, Integer> {
    ...
    public Integer apply(A value) {
        ...
        return switch (Ordering.compare(vhc, hc)) {
            case LT -> left.apply(value);
            case GT -> right.apply(value);
            case EQ -> bucketValue(value);
        };
    }
    ...
    public MultiSet<A> plus(A a, int value) {
        ...
        int ahc = a.hashCode();
        return switch (Ordering.compare(ahc, hc)) {
            case EQ -> new MultiSet<>(hc, bucketWithout(a).plus(T2.of(a, bucketValue(a) + value)), left, right);
            case LT -> {
                MultiSet<A> newLeft = left.plus(a, value);
                yield left == newLeft ? this : new MultiSet<>(hc, bucket, newLeft, right);
            }
            case GT -> {
                MultiSet<A> newRight = right.plus(a, value);
                yield right == newRight ? this : new MultiSet<>(hc, bucket, left, right.plus(a, value));
            }
        };
    }
    ...
    public MultiSet<A> minus(A a, int value) {
        ...
        int ahc = a.hashCode();
        return switch (Ordering.compare(ahc, hc)) {
            case EQ -> eqCase(bucketMinus(a, value));
            case LT -> {
    ...
    public MultiSet<A> remove(A a) {
        int ahc = a.hashCode();
        return switch (Ordering.compare(ahc, hc)) {
            case EQ -> eqCase(bucketWithout(a));
            case LT -> {
    ...
    private MultiSet<A> eqCase(List<T2<A, Integer>> newBucket) {
    ...
    private List<T2<A, Integer>> bucketMinus(A a, int value) {
        int v = bucketValue(a);
        return switch (Ordering.compare(v, value)) {
            case EQ, LT -> bucketWithout(a);
            case GT -> bucketWithout(a).plus(T2.of(a, v - value));
        };
    }
