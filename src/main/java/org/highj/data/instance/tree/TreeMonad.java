...
public interface TreeMonad extends Monad<Tree.µ> {
    @Override
    default <A, B> Tree<B> bind(__<Tree.µ, A> nestedA, Function<A, __<Tree.µ, B>> fn) {
    ...
    @Override
    default <A> Tree<A> pure(A a) {
        return Tree.newTree(a, List.empty());
    }

    @Override
    default <A, B> Tree<B> map(Function<A, B> fn, __<Tree.µ, A> nestedA) {
        Tree<A> tree = asTree(nestedA);
        return Tree.newLazyTree(fn.apply(tree.rootLabel), () -> tree.subForest().map(t -> asTree(map(fn, t))));
    }

    @Override
    default <A, B> Tree<B> ap(__<Tree.µ, Function<A, B>> fn, __<Tree.µ, A> nestedA) {
