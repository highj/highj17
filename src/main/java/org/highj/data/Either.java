...
public abstract class Either<A, B> implements __2<Either.µ, A, B> {
    ...
    public <C> C constant(C left, C right) {
        return either(Functions.constant(left), Functions.constant(right));
    }
    ...
    public <C, D> Either<C, D> bimap(Function<? super A, ? extends C> leftFn, Function<? super B, ? extends D> rightFn) {
        return either(a -> Either.Left(leftFn.apply(a)),
            b -> Either.Right(rightFn.apply(b)));
    }
    ...
    public <C> Either<C, B> leftMap(Function<? super A, ? extends C> leftFn) {
        return bimap(leftFn, Function.identity());
    }
    ...
    public <C> Either<A, C> rightMap(Function<? super B, ? extends C> rightFn) {
        return bimap(Function.identity(), rightFn);
    }
     ...
     * @return the Left instance
     */
    public static <A, B> Either<A, B> Left(final A a, @SuppressWarnings("unused") Class<B> clazz) {
        return Left(a);
    }
     ...
     * @return the Right instance.
     */
    public static <A, B> Either<A, B> Right(@SuppressWarnings("unused") Class<A> clazz, final B b) {
        return Right(b);
    }
    ...
    public Maybe<A> maybeLeft() {
        return either(Maybe::Just, Functions.constant(Maybe.Nothing()));
    }
    ...
    public Maybe<B> maybeRight() {
        return either(Functions.constant(Maybe.Nothing()), Maybe::Just);
    }
    ...
    public B rightOrElse(B defaultValue) {
        return either(Functions.constant(defaultValue), Function.identity());
    }
    ...
    public static <A> List<A> lefts(List<? extends Either<A, ?>> list) {
        return list.concatMap(a -> a.maybeLeft().asList());
    }
    ...
    public static <A> List<A> lazyLefts(List<? extends Either<A, ?>> list) throws StackOverflowError {
        if (list.isEmpty()) {
            ...
            return head.either(
                a -> List.Cons$(a, () -> Either.lazyLefts(list.tail())),
    ...
    public static <B> List<B> rights(List<? extends Either<?, B>> list) {
        return list.concatMap(a -> a.maybeRight().asList());
    }
    ...
    public static <B> List<B> lazyRights(List<? extends Either<?, B>> list) throws StackOverflowError {
        if (list.isEmpty()) {
            ...
            return head.either(
                a -> lazyRights(list.tail()),
                b -> List.Cons$(b, () -> Either.lazyRights(list.tail())));
        }
    }
     ...
     */
    public static final EitherBifoldable bifoldable = new EitherBifoldable() {
    };
